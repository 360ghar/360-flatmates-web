...
Both inboxes stay current without polling. `useFlatmatesRealtime` (`src/hooks/useFlatmatesRealtime.ts`) subscribes to the bootstrap-configured private Broadcast channel, and its invalidation switch handles a `new_match` event by invalidating these keys:
...
- `["incoming-likes"]`, `["outgoing-likes"]`, and `["conversations"]`, so related inboxes and the chat list reconcile.

This means the moment the backend records a mutual like, the matches list refetches and the new card slides in. The event itself carries no payload that the UI trusts for rendering; it is purely a cache-invalidation signal, and the query refetch supplies the authoritative data. See [Real-time](real-time.md) for the Broadcast lifecycle.

```mermaid
graph LR
    B["FastAPI backend"] -->|new_match Broadcast event| H["useFlatmatesRealtime hook"]
...
For completeness, the Broadcast handler also invalidates conversations on related message events (see [Messaging](messaging.md)). `new_match` is the event that moves a profile into the matches inbox and refreshes the likes/matches surfaces directly.
