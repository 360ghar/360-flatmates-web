...
| Bootstrap | `Bootstrap` | `GET /flatmates/bootstrap` (profile, catalogs, activity counts, and Supabase realtime config in one call) | `src/lib/api/user.types.ts` (`FlatmatesBootstrap`) |
...
| Real-time | `Bootstrap` | No standalone stream endpoint. `GET /flatmates/bootstrap` returns `realtime`, and the client subscribes to the configured Supabase private Broadcast channel. See [Real-time](../features/real-time.md). | `src/lib/api/user.types.ts` (`FlatmatesRealtimeConfig`) |
